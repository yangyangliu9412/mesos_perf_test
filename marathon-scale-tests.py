...
def check_in_deployment(app_name, timeout):
    ...
    while deployments != 0 and not is_timeout(start_time, timeout):
		...
		logging.info("LOG APP NAME: {}".format(
			     app_name))
                deployments = len(app.deployments)

		logging.info("LOG DEPLOYMENT: {}".format(
			     deployments))
    return None
...
def check_operation_status(start_time, app_name, app_instances,
    ...
    flag_time = time.time()
    logging.info("INSERT FLAG: current time is {}".format(flag_time))
    successful_instances = check_instances(
        "task", app_instances, app_name, start_time, timeout, ready_tasks)
    logging.info("FLAG1: current time is {}".format(
		 time.time()-flag_time))

    all_starting = []
    tasks = cluster.list_tasks(app_name)

    logging.info("FLAG2: current time is {}".format(
                 time.time()-flag_time))
    ...
    logging.info("FLAG3: current time is {}".format(
                 time.time()-flag_time))
    check_in_deployment(app_name, timeout)
   
    logging.info("FLAG4: current time is {}".format(
                 time.time()-flag_time))
    ...
    logging.info("END FLAG: current time is {}".format(app_full_time))
...
def create_app(app_instances, delete=False,
               timeout=20, need_statistics=True):
    """
